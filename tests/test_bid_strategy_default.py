...
When a budget is provided without an explicit bid_strategy (and no bid cap), Meta defaults
the object to LOWEST_COST_WITH_BID_CAP, which requires a bid_amount/bid_cap and 400s if absent.
The tools now default such calls to LOWEST_COST_WITHOUT_CAP (automatic bidding) so the create
call succeeds, while leaving explicit strategies, cap-bidding callers, and CBO ad sets untouched.
...
class TestCreateAdsetBidStrategyDefault:
    ...
    @pytest.mark.asyncio
    async def test_adset_budget_without_strategy_defaults_to_lowest_cost(
        self, mock_api_request, mock_auth_manager, base_params
    ):
        """Ad-set budget + no bid_strategy/bid_amount -> LOWEST_COST_WITHOUT_CAP is sent."""
        await create_adset(**base_params, daily_budget=100)
        params = _sent_params(mock_api_request)
        assert params["bid_strategy"] == "LOWEST_COST_WITHOUT_CAP"

    @pytest.mark.asyncio
    async def test_no_adset_budget_does_not_add_strategy(
        self, mock_api_request, mock_auth_manager, base_params
    ):
        """No ad-set budget (CBO ad set) -> bid_strategy not defaulted at ad-set level."""
    ...
    async def test_explicit_strategy_is_respected(
        ...
        """Explicit bid_strategy is never overridden by the default."""
        await create_adset(
            **base_params, daily_budget=100, bid_strategy="COST_CAP", bid_amount=200
        )
        params = _sent_params(mock_api_request)
        assert params["bid_strategy"] == "COST_CAP"

    @pytest.mark.asyncio
    async def test_bid_amount_without_strategy_is_not_overridden(
        self, mock_api_request, mock_auth_manager, base_params
    ):
        """A caller passing a bid_amount keeps cap bidding -> no forced LOWEST_COST_WITHOUT_CAP."""
        await create_adset(**base_params, daily_budget=100, bid_amount=200)
        params = _sent_params(mock_api_request)
        assert "bid_strategy" not in params
